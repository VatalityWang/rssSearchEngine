...
DirScanner::DirScanner(Configuration & conf)
	: conf_(conf)
{
	vecFiles_.reserve(kFileNo);//将给vector容器的容量变化到10000
}

void DirScanner::operator()()//重载（）运算符
{
	std::map<std::string, std::string> & confMap = conf_.getConfigMap();
	std::string dirName = confMap[YULIAO_KEY];
	traverse(dirName);//遍历目录并存进相应容器
}
...
void DirScanner::traverse(const std::string & dirName)
{
	...
	//进入指定的目录
	::chdir(dirName.c_str());//linux下系统接口
	...
	while(NULL != (mydirent = ::readdir(pdir)))
	{
		...
		else
		{
			...
			std::string fileName;
			fileName.append(::getcwd(NULL, 0))//获取当前路径
					.append("/")
