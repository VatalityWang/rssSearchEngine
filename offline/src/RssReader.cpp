...
void RssReader::makePages(vector<string> & pages)
{
	...
	{
		ostringstream oss;//向string流中写入数据
		oss << "<doc>\n"
			<< "  <docid>" << (idx + 1) << "</docid>\n"
...
void RssReader::parseRss(XMLDocument & doc)//解析xml文件，形成存放网页库的智能指针
{
	...
	{
		shared_ptr<RssItem> rssItemPtr(new RssItem);//定义一个指向rssItem结构体的智能指针的vector容器
		...
		string content = boost::regex_replace(string(iContent), fmt, string(""));//字符串处理函数
